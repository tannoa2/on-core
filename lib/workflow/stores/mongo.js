...
function mongoStoreFactory(waterline, Promise, Constants, Errors, assert, _, sanitizer) {
     ...
     */
        ...
        .then(function(taskDefinition) {
            ...
            var t= taskDefinition.toJSON();
            //console.log (t)
            return t
        });
    };

    exports.getTaskDefinition1 = function(injectableName) {
        return  new Promise(function(resolve, reject){
                        _db.collection('taskdefinitions').findOne({injectableName: injectableName }, function (err, taskDefinition) {
    ...
    */
    ...
    exports.getGraphDefinitions1 = function(injectableName) {
        var t = []
        var query = {};
        if (injectableName) {
            query.injectableName = injectableName;
        ...
        return  new Promise(function(resolve, reject){
            _db.collection('graphdefinitions').find(query)
    ...
    exports.persistGraphObject1 = function(graph) {
    ...
    exports.persistGraphObject = function(graph) {
        console.time('persistGraphObject')
        console.time('waterline.taskdependencies.mongo.objectId')
        assert.object(graph, 'graph');var start = new Date().getTime()
        assert.uuid(graph.instanceId, 'graph.instanceId');

        if (graph.node) {
            graph.node = waterline.taskdependencies.mongo.objectId(graph.node)
                .tap(console.timeEnd.bind(console,'waterline.taskdependencies.mongo.objectId'))
        }
        ...
        var sum = 0
        return waterline.graphobjects.findAndModifyMongo(query, {}, graph, options)
            .tap( console.timeEnd.bind(console,'waterline_graphobjects_findAndModifyMongo'))
            .tap(console.timeEnd.bind(console,'persistGraphObject'))
            .then (function(){

                var end = new Date().getTime()
                var e = end-start
                //console.log("waterline.graphobjects.findAndModifyMongof" + e)
                waterline_graphobjects_findAndModifyMongoARR.push(e)
                //console.log("waterline_graphobjects_findAndModifyMongoARR.length" = waterline_graphobjects_findAndModifyMongoARR.length)

                if(waterline_graphobjects_findAndModifyMongoARR.length == 1000 ) {
                    waterline_graphobjects_findAndModifyMongoARR.forEach(function (item) {
                        sum = sum + item
                    })
                    ...
                    console.log("time_array of waterline_graphobjects_findAndModifyMongoARR: " + waterline_graphobjects_findAndModifyMongoARR)
                    console.log("time_avg of waterline_graphobjects_findAndModifyMongoAVG: " + waterline_graphobjects_findAndModifyMongoAVG)
                }
     ...
     * @memberOf store
     */
    var persistTaskDependenciesAVG =0
    var persistTaskDependenciesARR = []
    exports.persistTaskDependencies = function(taskDependencyItem, graphId) {
        console.time('persistTaskDependencies')
        ...
        var w= waterline.taskdependencies.create(obj)
            .then(function(w1){
                console.log(w1)
                var e
                return w1
            })
        console.timeEnd('persistTaskDependencies')
        return w
    };
    exports.persistTaskDependencies1 = function(taskDependencyItem, graphId) {
        console.time('persistTaskDependencies1')
        assert.object(taskDependencyItem, 'taskDependencyItem');
        assert.uuid(taskDependencyItem.taskId, 'taskDependencyItem.taskId');
        assert.uuid(graphId, 'graphId');
        assert.object(taskDependencyItem.dependencies, 'taskDependencyItem.dependencies');
        ...
        //var w= waterline.taskdependencies.create(obj)
        console.timeEnd('persistTaskDependencies1')
        return obj
    };
    ...
    */
    exports.findReadyTasks = function(domain, graphId) {
        ...
        .filter(function(task) {
            ...
            var unmetDeps = [];
            var promises = [];
            _.forEach(task.dependencies, function(state, taskId) {
                console.log(' 2 ******** ', taskId, state);
                var p = Promise.try(function() {
                    return waterline.taskdependencies.findOne({'taskId': taskId})
                })
                .tap(function(depTask) {
                    console.log(' 3 ******** ', depTask.state);
                    if (!_.includes(Constants.Task.FinishedStates, depTask.state)) {
                        // Dependency is not in a finished state
                        unmetDeps.push(depTask.taskId);
                    } else if (state !== 'finished' && depTask.state !== state) {
                        unmetDeps.push(depTask.taskId);
                    }
                })
                promises.push(p);   
